...
namespace WpfWorldMapDisplay
{
    public class BallDisplay
    {
        ...
        public BallDisplay(LocationExtended locExt)
        {
            try
            {
                location = new Location(locExt.X, locExt.Y, locExt.Theta, locExt.Vx, locExt.Vy, locExt.Vtheta);
            }
            catch { }
        }
        ...
        public PolygonExtended GetBallPolygon()
        {
            ...
            if (location != null)
            {
                ...
                {
                    PointD pt = new PointD(radius * Math.Cos(theta), radius * Math.Sin(theta));
                    pt.X += location.X;
                    pt.Y += location.Y;
        ...
        public PolygonExtended GetBallSpeedArrow()
        {
            ...
            if (location != null)
            {
                ...
                polygonToDisplay.polygon.Points.Add(new PointD(location.X, location.Y));
                polygonToDisplay.polygon.Points.Add(new PointD(location.X + xTete, location.Y + yTete));
                double angleTeteFleche1 = headingAngle + angleTeteFleche;
                double angleTeteFleche2 = headingAngle - angleTeteFleche;
                polygonToDisplay.polygon.Points.Add(new PointD(location.X + xTete - longueurTeteFleche * Math.Cos(angleTeteFleche1), location.Y + yTete - longueurTeteFleche * Math.Sin(angleTeteFleche1)));
                polygonToDisplay.polygon.Points.Add(new PointD(location.X + xTete, location.Y + yTete));
                polygonToDisplay.polygon.Points.Add(new PointD(location.X + xTete - longueurTeteFleche * Math.Cos(angleTeteFleche2), location.Y + yTete - longueurTeteFleche * Math.Sin(angleTeteFleche2)));
                polygonToDisplay.polygon.Points.Add(new PointD(location.X + xTete, location.Y + yTete));
